...
namespace GammaClient.GCFacilities.WCFProxy
{
    ...
    class ProxyFacadeCmdExecutor
    {
        ...
        public static string ExecuteCmdSync(IMachineInfo machine, string single_command)
        {
        ...
        public static List<Task<string>> ExecuteCmdParallelOnNodesAsync(IEnumerable<IMachineInfo> machines, string single_command)
        {
            List<Task<string>> tasks = new List<Task<string>>();
            foreach (IMachineInfo machine in from m in machines where m.IsSelected && m.Alive == NodeState.Online select m)
            {
